...
public class OVRGrabber : MonoBehaviour
{
    ...
    protected virtual void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
    {
        ...
        float yRot = m_grabbedObjectRotOff.eulerAngles.y;
        ...
        {
            /*
             * Location to disable the abilty to move and only rotate a object.
             */
            if (m_grabbedObj.RotateOnly())
            {
                Vector3 rotAngle = grabbableRotation.eulerAngles;
                rotAngle.x = 0;
                if (m_grabbedObj.tag.Equals("Left"))
                {
                    rotAngle.y = 0;
                }
                else if (m_grabbedObj.tag.Equals("Right"))
                {
                    rotAngle.y = 180;
                }
                else if (m_grabbedObj.tag.Equals("Swing"))
                {
                    rotAngle.z = 0;
                    rotAngle.y += grabbablePosition.z;
                    Debug.Log(grabbablePosition.z);
                    rotAngle.y += grabbablePosition.x;
                    rotAngle.y += grabbablePosition.y;
                }
                grabbableRotation = Quaternion.Euler(rotAngle);
            }
            else
            {
