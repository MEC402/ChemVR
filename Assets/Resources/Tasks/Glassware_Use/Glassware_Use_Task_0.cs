...
public class Glassware_Use_Task_0 : TaskStep
{
    public void Start()
    {
    ...
    public void Restart()
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            if (child.name.Contains("gla") && !child.name.Contains("0"))
            {
                Destroy(child);
            }
            else if (!child.name.Contains("gla"))
            {
                Destroy(child);
            }
        }
    ...
    public void Abandon()
    {
        GameObject destroy = GameObject.Find("Glassware_Use_Task_0(Clone)");
        Destroy(destroy);
    }
